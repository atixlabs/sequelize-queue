...
var Queue = function (sequelize, options) {
	...
	this.options = options = _.extend({
		...
		logger: new Log(-1),
		polltime: 1000,
		processRunning: options.processRunning || false
	}, options || {});
...
_.extend(Queue.prototype, {
	...
	stop: function () {
		...
		LOGGER.info('Stoping queue %s. Running? %s', this.model.name, this.running !== undefined);
		if (this.running) {
			return this.running.then(() => {
				LOGGER.info('Queue %s stopped', this.model.name);
				return true;
			})
		}
...
function runDelayed(queue) {
	...
	setTimeout(() => {
		LOGGER.debug('Automatic %s queue run', queue.model.name)
		runQueue(queue)
	...
	queue.running = Promise.try(function () {
		...
		return queue.model.find({
			where: Sequelize
				...
				Sequelize.or({ running: false }, { running: queue.options.processRunning }),
				Sequelize.or({ dateNextRun: { $eq: null } }, { dateNextRun: { $lt: new Date() } })),
			order: [
				['priority', 'DESC'],
				['dateAdded', 'ASC']
			]
		...
		.then(function (job) {
			...
			if (!job) {
				LOGGER.debug('%s - No job to run found', queue.model.name)
				return;
			}

			LOGGER.debug('%s - Running job %s', queue.model.name, job.id)
			// run the job
			return queue.processors[job.type](JSON.parse(job.data))
				.then(function (result) {
					...
					LOGGER.debug('%s - Saving job %s', queue.model.name, job.id)
				...
				.catch(function (err) {
					...
					LOGGER.error('%s - Job %s failed', queue.model.name, job.id)
					let endAction
					if ((job.retryInterval !== null) && (err.name !== 'JobFailedError')) {
						...
						LOGGER.info('%s - Job %s will be re-runned at', queue.model.name, job.id, nextRun)
						endAction = job.save()
					} else {
						job.failed = true;
						job.dateFinished = new Date();
			...
			job ? LOGGER.info('%s - Finish job %s run', queue.model.name, job.id) : LOGGER.debug('No job run this time')
